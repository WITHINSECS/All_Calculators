...
export default function Page() {
  ...
  const calculateGST = () => {
    const amountNum = parseFloat(amount); // Ensure amount is a number
    if (amountNum <= 0 || gstPercentage <= 0) {
    ...
    if (taxType === "exclusive") {
      const gst = (amountNum * gstPercentage) / 100;
      setGstAmount(parseFloat(gst.toFixed(2)));
      setTotalAmount(parseFloat((amountNum + gst).toFixed(2)));
      setActualAmount(amountNum); // For exclusive GST, actual amount is the entered amount
    } else { // Inclusive GST
      const gst = (amountNum * gstPercentage) / (100 + gstPercentage);
      setGstAmount(parseFloat(gst.toFixed(2)));
      setTotalAmount(parseFloat(amountNum.toFixed(2)));
      // Correct calculation for actual amount when GST is inclusive
      const actual = amountNum / (1 + gstPercentage / 100);
      setActualAmount(parseFloat(actual.toFixed(2)));
    }
  };
