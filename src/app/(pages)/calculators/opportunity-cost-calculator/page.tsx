...
export default function OpportunityCostCalculator(): JSX.Element {
  ...
  // Returns & period (now empty by default)
  const [annualReturnInput, setAnnualReturnInput] = useState<string>(""); // %
  const [yearsInput, setYearsInput] = useState<string>("");
  const [monthsInput, setMonthsInput] = useState<string>("");

  // Taxes & inflation (now empty by default)
  const [taxInput, setTaxInput] = useState<string>(""); // %
  const [inflationInput, setInflationInput] = useState<string>(""); // %
  ...
  const validate = (): {
    ...
    if (principalInput === "") {
      toast.error("Enter an amount to invest.");
      return null;
    }
    const P = Number(principalInput);
    const rPct = Number(annualReturnInput);
    const y =
      (Number(yearsInput || 0) || 0) + (Number(monthsInput || 0) || 0) / 12;
    ...
    if (!Number.isFinite(rPct) || rPct < 0) {
      toast.error("Annual return cannot be negative.");
      return null;
    }
    if (!Number.isFinite(y) || y <= 0) {
      toast.error("Investment period must be greater than 0.");
      return null;
    }
    if (!Number.isFinite(tax) || tax < 0) {
      toast.error("Income tax cannot be negative.");
      return null;
    }
    if (!Number.isFinite(inf) || inf < 0) {
      toast.error("Inflation rate cannot be negative.");
      return null;
    }
      ...
      <div className="mx-auto md:mt-16 p-5 mt-8 max-w-4xl">
        <div className="mb-6 text-center">
          ...
          <p className="text-muted-foreground mt-3 text-lg">
            How much could your money grow after taxes and inflation?
          </p>
        </div>
              ...
              {results ? (
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <KPI
                      label="Total savings (after tax)"
                      value={fmt(results.fvAfterTax, currency)}
                    />
                    <KPI
                      label="Opportunity cost — inflation adjusted"
                      value={fmt(results.opportunityCostReal, currency)}
                    />
                  </div>
                      ...
                      >
                        <defs>
                          <linearGradient id="fillNom" x1="0" y1="0" x2="0" y2="1">
                            <stop
                              offset="5%"
                              stopColor="hsla(var(--primary),0.6)"
                            />
                            <stop
                              offset="95%"
                              stopColor="hsla(var(--primary),0.1)"
                            />
                          </linearGradient>
                          <linearGradient id="fillReal" x1="0" y1="0" x2="0" y2="1">
                            <stop
                              offset="5%"
                              stopColor="hsla(var(--secondary),0.6)"
                            />
                            <stop
                              offset="95%"
                              stopColor="hsla(var(--secondary),0.1)"
                            />
                          </linearGradient>
                        </defs>
                      ...
                      <TableBody>
                        <TableRow>
                          ...
                          <TableCell className="text-right">
                            {fmt(Number(principalInput || 0), currency)}
                          </TableCell>
                        </TableRow>
                        <TableRow>
                          <TableCell>Interest earned (after tax)</TableCell>
                          <TableCell className="text-right">
                            {fmt(results.interestAfterTax, currency)}
                          </TableCell>
                        </TableRow>
                        <TableRow>
                          <TableCell>Inflation-adjusted future value</TableCell>
                          <TableCell className="text-right">
                            {fmt(results.realFV, currency)}
                          </TableCell>
                        </TableRow>
                      </TableBody>
                    </Table>
                ...
                <div className="text-center text-muted-foreground py-16">
                  Enter your inputs and click <b>Calculate</b> to see totals and the
                  growth curve.
                </div>
              )}
            </CardContent>
