...
export default function EVTravelCostCalculator(): JSX.Element {
  // Currency (starts empty, select uses placeholder)
  const [currency, setCurrency] = useState<Currency | "">("");

  // EV inputs (text inputs start empty, sliders keep numeric mirrors)
  const [evRangeIn, setEvRangeIn] = useState<string>(""); // km
  const [evRange, setEvRange] = useState<number>(200);

  const [batteryKWhIn, setBatteryKWhIn] = useState<string>("");
  const [batteryKWh, setBatteryKWh] = useState<number>(15);

  const [distIn, setDistIn] = useState<string>("");
  const [distanceKm, setDistanceKm] = useState<number>(100);

  const [pricePerKWhIn, setPricePerKWhIn] = useState<string>("");
  const [pricePerKWh, setPricePerKWh] = useState<number>(0.12);

  // ICE inputs
  const [fuelKind, setFuelKind] = useState<FuelKind | "">("");
  const [fuelPriceIn, setFuelPriceIn] = useState<string>(""); // per liter
  const [fuelPrice, setFuelPrice] = useState<number>(4);
  const [mileageIn, setMileageIn] = useState<string>(""); // km/litre
  ...
  const calculate = (): void => {
    ...
    if (!currency || !fuelKind) {
      toast.error("Please select currency and fuel type before calculating.");
      return;
    }
    ...
    setResults({
      ...
      evCost,
      iceCost,
      savings: Math.max(0, iceCost - evCost),
      evKWhPer100: kWhPer100,
      iceLPer100: LPer100,
    });
  };
  ...
  const clearAll = (): void => {
    ...
    // reset sliders to defaults (they don't show numbers directly)
    setEvRange(200);
    setBatteryKWh(15);
    setDistanceKm(100);
    setPricePerKWh(0.12);
    ...
    // reset selects to empty / placeholder
    setCurrency("");
    setFuelKind("");

    // clear results (no chart / values)
    setResults(null);
  };
              ...
              <Tabs defaultValue="ev" className="w-full">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      <Input
                        ...
                        value={evRangeIn}
                        onChange={(e) => {
                          setEvRangeIn(e.target.value);
                          syncNumber(e.target.value, setEvRange);
                        }}
                      />
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      <Input
                        ...
                        value={batteryKWhIn}
                        onChange={(e) => {
                          setBatteryKWhIn(e.target.value);
                          syncNumber(e.target.value, setBatteryKWh);
                        }}
                      />
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      <Input
                        ...
                        value={distIn}
                        onChange={(e) => {
                          setDistIn(e.target.value);
                          syncNumber(e.target.value, setDistanceKm);
                        }}
                      />
                      <span className="text-sm text-muted-foreground">km</span>
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      <Input
                        ...
                        value={pricePerKWhIn}
                        onChange={(e) => {
                          setPricePerKWhIn(e.target.value);
                          syncNumber(e.target.value, setPricePerKWh);
                        }}
                      />
                      <span className="text-sm text-muted-foreground">
                        {currency || "Currency"} / kWh
                      </span>
                    </div>
                    <Slider
                      value={[pricePerKWh]}
                ...
                <TabsContent value="ice" className="space-y-5 pt-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={fuelKind || undefined}
                      onValueChange={(v: FuelKind) => setFuelKind(v)}
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      <Input
                        ...
                        value={fuelPriceIn}
                        onChange={(e) => {
                          setFuelPriceIn(e.target.value);
                          syncNumber(e.target.value, setFuelPrice);
                        }}
                      />
                      <span className="text-sm text-muted-foreground">
                        {currency || "Currency"} / litre
                      </span>
                    </div>
                    <Slider
                      value={[fuelPrice]}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      <Input
                        ...
                        value={mileageIn}
                        onChange={(e) => {
                          setMileageIn(e.target.value);
                          syncNumber(e.target.value, setMileageKmPerL);
                        }}
                      />
                    ...
                    <Slider
                      value={[mileageKmPerL]}
                      onValueChange={(v) => {
                        setMileageKmPerL(v[0]);
                        setMileageIn(String(v[0]));
                      }}
                      min={5}
                      max={40}
                      step={0.5}
              ...
              {results ? (
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <KPI
                      label={`Trip distance`}
                      value={`${results.distanceKm.toLocaleString()} km`}
                    />
                    <KPI
                      label={`EV travel cost`}
                      value={fmtMoney(results.evCost, currency as Currency)}
                    />
                    <KPI
                      label={`Conventional travel cost`}
                      value={fmtMoney(results.iceCost, currency as Currency)}
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        >
                          ...
                          <YAxis
                            tickFormatter={(v: number) =>
                              fmtMoney(v, currency as Currency)
                            }
                            width={90}
                          />
                          <Tooltip
                    ...
                    <div className="border rounded-md p-4 space-y-2">
                      ...
                      <div className="text-xl font-semibold">
                        EV: {results.evKWhPer100.toFixed(1)} kWh / 100 km
                      </div>
                      <div className="text-xl font-semibold">
                        ICE: {results.iceLPer100.toFixed(1)} L / 100 km
                      </div>
                      <Separator className="my-2" />
                      <div className="text-sm text-muted-foreground">
                        Estimated savings this trip
                      </div>
                      <div className="text-2xl font-semibold">
                        {fmtMoney(results.savings, currency as Currency)}
                      </div>
                    </div>
                  </div>
                      ...
                      <TableBody>
                        <TableRow>
                          ...
                          <TableCell className="text-right">
                            {fmtMoney(results.evCost, currency as Currency)}
                          </TableCell>
                        </TableRow>
                        <TableRow>
                          <TableCell>
                            Conventional cost for {results.distanceKm} km
                          </TableCell>
                          <TableCell className="text-right">
                            {fmtMoney(results.iceCost, currency as Currency)}
                          </TableCell>
                        </TableRow>
                        <TableRow>
                ...
                <div className="text-center text-muted-foreground py-16">
                  Adjust inputs on the left and click <b>Calculate</b> to compare costs.
                </div>
              )}
            </CardContent>
          </Card>
