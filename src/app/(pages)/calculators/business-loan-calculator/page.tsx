...
const periodsPerYear = (p: Payback): number => (p === "weekly" ? 52 : p === "biweekly" ? 26 : 12);

const compPerYear = (c: Compound): number => (c === "monthly" ? 12 : c === "quarterly" ? 4 : 1);
...
const irrPerPeriod = (netProceeds: number, payment: number, n: number): number | null => {
    ...
    let lo = 0, hi = 1; // 0%..100% per period
...
const CHART_COLORS = ["hsl(var(--primary))", "hsl(var(--secondary))", "hsl(var(--destructive))"];
...
export default function BusinessLoanCalculator(): JSX.Element {
    ...
    const [originationPct, setOriginationPct] = useState<string>("0");
    const [docFeeInput, setDocFeeInput] = useState<string>("0");
    ...
    const validate = () => {
        ...
        if (!Number.isFinite(A) || A <= 0) { toast.error("Loan amount must be greater than 0."); return null; }
        if (!Number.isFinite(rPct) || rPct < 0) { toast.error("Interest rate cannot be negative."); return null; }
        if (!Number.isFinite(yrs) || !Number.isFinite(mos) || (yrs <= 0 && mos <= 0)) { toast.error("Loan term must be greater than 0."); return null; }
        if (org < 0 || doc < 0 || oth < 0) { toast.error("Fees cannot be negative."); return null; }
    ...
    const onCalculate = (): void => {
        ...
        if (n <= 0) { toast.error("Loan term is too short for the selected payback."); return; }

        const rPer = perPaymentRate(rPct, compound, payback);
        const payment = pmt(A, rPer, n);
        if (!Number.isFinite(payment)) { toast.error("Payment computation failed. Check inputs."); return; }
        ...
        setResults({ paymentEach: payment, totalPayments: totalPaid, interestOnly: interest, upfrontFees: fees, interestPlusFees, realAPR, breakdown });
    };
        ...
        <Wrapper>
            <div className="mx-auto md:mt-16 p-5 mt-8 max-w-6xl">
                <div className="text-center mb-6">
                    <h1 className="text-2xl font-semibold lg:text-4xl">Business Loan Calculator</h1>
                    <p className="text-muted-foreground mt-3 text-lg">Clean design with pie breakdown of principal, interest, and fees.</p>
                </div>
                        ...
                        <CardContent className="space-y-5">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <Input type="number" placeholder="e.g. 10000" value={amountInput} onChange={(e) => setAmountInput(e.target.value)} />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-3">
                                    <Input type="number" placeholder="e.g. 10" value={rateInput} onChange={(e) => setRateInput(e.target.value)} />
                            ...
                            <div className="space-y-2">
                                ...
                                <Select value={compound} onValueChange={(v: Compound) => setCompound(v)}>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="flex items-center gap-2">
                                        <Input type="number" placeholder="years" value={yearsInput} onChange={(e) => setYearsInput(e.target.value)} />
                                        <span className="text-sm text-muted-foreground">years</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Input type="number" placeholder="months" value={monthsInput} onChange={(e) => setMonthsInput(e.target.value)} />
                            ...
                            <div className="space-y-2">
                                ...
                                <Select value={payback} onValueChange={(v: Payback) => setPayback(v)}>
                                    <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                                    <SelectContent>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <Input type="number" value={originationPct} onChange={(e) => setOriginationPct(e.target.value)} />
                                        <div className="px-3 py-2 rounded-md border text-sm">%</div>
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <Label>Documentation fee</Label>
                                    <Input type="number" value={docFeeInput} onChange={(e) => setDocFeeInput(e.target.value)} />
                                </div>
                                <div className="space-y-2">
                                    <Label>Other fees</Label>
                                    <Input type="number" value={otherFeeInput} onChange={(e) => setOtherFeeInput(e.target.value)} />
                                </div>
                            </div>
                            ...
                            {results ? (
                                <>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <KPI label={`Payback every ${payback === "monthly" ? "month" : payback === "biweekly" ? "2 weeks" : "week"}`} value={fmt(results.paymentEach, currency)} />
                                        <KPI label={`Total of ${Math.round((Number(yearsInput || 0) + Number(monthsInput || 0) / 12) * periodsPerYear(payback))} payments`} value={fmt(results.totalPayments, currency)} />
                                        <KPI label="Interest" value={fmt(results.interestOnly, currency)} />
                                        <KPI label="Interest + fees" value={fmt(results.interestPlusFees, currency)} />
                                        <KPI label="Real rate (APR)" value={results.realAPR != null ? `${results.realAPR.toFixed(3)}%` : "—"} />
                                    </div>

                                    <div className="h-64">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie data={results.breakdown} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={90}>
                                                    {results.breakdown.map((_, i) => (
                                                        <Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />
                                                    ))}
                                                </Pie>
                                                <Legend />
                                                <Tooltip formatter={(v: number | string) => (typeof v === "number" ? fmt(v, currency) : String(v))} />
                                            </PieChart>
                                        </ResponsiveContainer>
                                    </div>
                                        ...
                                        <Table>
                                            ...
                                            <TableBody>
                                                <TableRow>
                                                    ...
                                                    <TableCell className="text-right">{fmt(results.upfrontFees, currency)}</TableCell>
                                                </TableRow>
                                                <TableRow>
                                                    <TableCell>Total interest</TableCell>
                                                    <TableCell className="text-right">{fmt(results.interestOnly, currency)}</TableCell>
                                                </TableRow>
                                                <TableRow>
                                                    <TableCell>Total paid</TableCell>
                                                    <TableCell className="text-right">{fmt(results.totalPayments, currency)}</TableCell>
                                                </TableRow>
                                            </TableBody>
                                        </Table>
                                    </div>
                                ...
                                <div className="text-center text-muted-foreground py-16">
                                    Enter the loan details and click <b>Calculate</b> to see payments and APR.
                                </div>
                            )}
                        </CardContent>
                    </Card>
