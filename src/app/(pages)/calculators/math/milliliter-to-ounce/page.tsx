"use client";

import { useState } from "react";
import Wrapper from "@/app/Wrapper";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";

// Conversion factor (1 milliliter = 0.033814 US liquid ounces)
const mlToOz = 0.033814;

export default function MlToOzCalculator() {
    const [milliliters, setMilliliters] = useState<number | string>("");
    const [ounces, setOunces] = useState<number | null>(null);
    const [error, setError] = useState<string | null>(null);
    const [steps, setSteps] = useState<string | null>(null);

    const onCalculate = () => {
        setError(null);
        setOunces(null);
        setSteps(null);

        if (milliliters === "") {
            setError("Please enter a value in milliliters.");
            return;
        }

        const ml = Number(milliliters);
        if (isNaN(ml) || ml < 0) {
            setError("Please enter a valid number for milliliters.");
            return;
        }

        const oz = ml * mlToOz;
        setOunces(oz);

        // Join the array into a single string before setting it
        setSteps(`
    Milliliters: ${ml}
    Formula: Milliliters × 0.033814 = Ounces
    Ounces = ${ml} × 0.033814 = ${oz.toFixed(4)}
  `);
    };

    const onClear = () => {
        setMilliliters("");
        setError(null);
        setOunces(null);
        setSteps(null);
    };

    return (
        <Wrapper>
            <div className="container mx-auto p-5 lg:px-12 md:my-14 my-8 max-w-4xl">
                <div className="mb-6">
                    <h1 className="text-2xl font-semibold lg:text-4xl">Milliliter to Ounce (US, liquid) Calculator</h1>
                </div>

                <div className="space-y-3">
                    <Label htmlFor="ml">Enter Milliliters</Label>
                    <div className="flex gap-2">
                        <Input
                            id="ml"
                            className="flex-1"
                            placeholder="Milliliters (e.g., 100)"
                            type="number"
                            value={milliliters}
                            onChange={(e) => setMilliliters(e.target.value)}
                        />
                        <Button onClick={onCalculate}>Calculate</Button>
                    </div>
                </div>

                {error && <p className="mt-6 text-sm text-red-600">{error}</p>}

                {ounces !== null && (
                    <div className="mt-8 space-y-4">
                        <div className="rounded-md border p-4">
                            <div className="text-sm text-muted-foreground mb-1">Result</div>
                            <div className="text-xl font-semibold">{ounces.toFixed(4)} US liquid ounces</div>
                        </div>

                        {steps && (
                            <div className="rounded-md border p-4">
                                <div className="mb-2 font-medium">Steps</div>
                                <pre className="whitespace-pre-wrap font-mono text-sm">
                                    {steps} {/* Directly render the string */}
                                </pre>
                            </div>
                        )}

                        <div className="flex gap-2">
                            <Button onClick={onClear} variant="secondary">Clear</Button>
                        </div>
                    </div>
                )}
            </div>
        </Wrapper>
    );
}
