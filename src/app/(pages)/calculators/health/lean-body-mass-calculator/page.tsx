...
// Form state can hold empty strings
interface LBMFormState {
  gender: string;
  age: number | "";
  height: number | "";
  weight: number | "";
...
const calculateLBM = (input: LBMInput): LBMResult => {
  ...
  const chartData: { age: number; lbm: number; fatMass: number }[] = [];
...
export default function LBMCalculator() {
  const [input, setInput] = useState<LBMFormState>({
    gender: "male",
    age: "",
    height: "",
    weight: "",
  ...
  const handleSelectChange = (name: keyof LBMFormState, value: string) => {
  ...
  const handleCalculate = () => {
    ...
    if (input.height === "" || Number(input.height) <= 0) {
    ...
    setError(null);
    const calcResult = calculateLBM(numericInput);
          ...
          {result && (
            <>
              <h3 className="text-xl">
                Your Lean Body Mass (LBM): {result.lbm.toFixed(2)} kg
              </h3>
              <h3 className="text-xl">
                Fat Mass: {result.fatMass.toFixed(2)} kg
              </h3>
