...
const calculateTargetHeartRate = (age: number, restingHeartRate: number) => {
  ...
  const targetRates: TargetHeartRateRanges = {
    "Very Light": {
      min: restingHeartRate + 0.5 * heartRateReserve,
      max: restingHeartRate + 0.6 * heartRateReserve,
    },
    Light: {
      min: restingHeartRate + 0.6 * heartRateReserve,
      max: restingHeartRate + 0.7 * heartRateReserve,
    },
    Moderate: {
      min: restingHeartRate + 0.7 * heartRateReserve,
      max: restingHeartRate + 0.8 * heartRateReserve,
    },
    Hard: {
      min: restingHeartRate + 0.8 * heartRateReserve,
      max: restingHeartRate + 0.9 * heartRateReserve,
    },
    "VO2 Max": {
      min: restingHeartRate + 0.9 * heartRateReserve,
      max: restingHeartRate + heartRateReserve,
    },
  };
...
const TargetHeartRateCalculator = () => {
  // keep inputs as strings so they can be empty
  const [age, setAge] = useState<string>("");
  const [restingHeartRate, setRestingHeartRate] = useState<string>("");
  const [targetRates, setTargetRates] = useState<TargetHeartRateRanges | null>(null);
  ...
  const handleCalculate = () => {
    ...
    const a = Number(age);
    const r = Number(restingHeartRate);
    ...
    const { targetRates } = calculateTargetHeartRate(a, r);
  ...
  return (
    <Wrapper>
      <div className="container mx-auto p-5 lg:px-12 md:my-14 my-8">
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:gap-10 md:grid-cols-2 gap-4">
              ...
              <div>
                <Label className="block mb-1.5" htmlFor="restingHeartRate">
        ...
        {/* Only show result section after a successful calculation */}
        {targetRates && (
          <div className="mt-8">
              ...
              <TableBody>
                {Object.keys(targetRates).map((intensity, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      {targetRates[intensity].min.toFixed(0)} -{" "}
                      {targetRates[intensity].max.toFixed(0)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
